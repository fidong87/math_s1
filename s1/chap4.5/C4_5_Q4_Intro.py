...
class C4_5_Q4_Intro(Scene):
    def construct(self):
      ...
      text.append(Text("使得").move_to(text[0]).shift(1.5*DOWN, 3.2*LEFT))
      self.play(Write(text[1]) , run_time=2) 
      text.append(MathTex(r"\left\{  \begin{array}{c} a=x+y, \\ b=xy, \end{array}").move_to(
      text[1]).shift(3*RIGHT))
      text[2].scale(1.5)
      text[2][0][7:12].shift(0.43*LEFT)    
      self.play(Write(text[2]),run_time=5)    

      text.append(Text("那么").move_to(text[1]).shift(1.5*DOWN))
      ...
      text.append(MathTex("=","\\sqrt{(\\sqrt{x} - \\sqrt{y})^2}"
          ).move_to(text[4]).shift(1.5*DOWN, 0.95*RIGHT))
      self.play(Write(text[5]) , run_time=5)
      self.wait()

      text.append(MathTex("=","\\sqrt{x} - \\sqrt{y}}"
          ).move_to(text[5]).shift(1.3*DOWN, 0.55*LEFT))
